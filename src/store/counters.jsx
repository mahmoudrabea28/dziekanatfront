...
export function CountersProvider({children}){
  ...
  const refreshCounts=useCallback(async(role)=>{
    try{
      if(role==='author'){ const r=await api.get('/notifications/unread-count'); setNotifCount(r.data.count||0) }
      if(role==='mentor'){ const r=await api.get('/articles/assigned/unseen-count'); setAssignedCount(r.data.count||0) }
    }catch(_){}
  },[])
